...
export default function AuthScreen() {
	...
	return (
		...
		>
			...
			>
				{/* Language Selector */}
				<ThemedView style={[styles.languageContainer, { paddingTop: insets.top + 10 }]}>
				...
				<ThemedView style={styles.form}>
					...
					<Button onPress={handleSubmit} disabled={loading === 'loading'} variant="primary" style={styles.submitButton}>
						{loading === 'loading' ? (
							<>
					...
					>
						<ThemedText style={[styles.toggleText, { color: tintColor }]}> {isLogin ? t('auth.register.link') : t('auth.login.link')} </ThemedText>
					</TouchableOpacity>

					<ThemedView style={styles.divider}>
						<ThemedView style={[styles.dividerLine, { backgroundColor: borderColor }]} />
						<ThemedText style={[styles.dividerText, { color: textColor }]}> {t('auth.or')} </ThemedText>
						<ThemedView style={[styles.dividerLine, { backgroundColor: borderColor }]} />
					</ThemedView>

					<Button onPress={handleAnonymousLogin} disabled={loading === 'loading'} variant="outline" style={styles.anonymousButton}>
						{t('auth.anonymous.button')}
					</Button>

					<Button onPress={handleShowOnboarding} disabled={loading === 'loading'} variant="secondary" style={styles.onboardingButton}>
